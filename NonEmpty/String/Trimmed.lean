...
/-! ### Bridge: Slice startsWith/endsWith matches its toString -/

theorem Slice.startsWith_copy (p : Char → Bool) (s : Slice) : s.copy.toSlice.startsWith p = s.startsWith p := by
  simp only [Slice.startsWith, String.startsWith, ForwardPattern.startsWith]
  -- Both sides reduce to checking front?
  cases h : s.front?
  · simp [show s.copy.toSlice.front? = none from Slice.copy_front? s ▸ h]
  · simp [show s.copy.toSlice.front? = some val from Slice.copy_front? s ▸ h]

theorem Slice.endsWith_copy (p : Char → Bool) (s : Slice) : s.copy.toSlice.endsWith p = s.endsWith p := by
  sorry -- same structure, needs copy_back?

theorem Slice.copy_front? (s : Slice) : s.copy.toSlice.front? = s.front? := by
  sorry


theorem startsWith_slice_toString {p : Char → Bool} (sl : Slice) :
    sl.toString.startsWith p = sl.startsWith p := by
  rw [startsWith_eq_startsWith_toSlice, Slice.toString_toSlice]

theorem endsWith_slice_toString {p : Char → Bool} (sl : Slice) :
    sl.toString.endsWith p = sl.endsWith p := by
  rw [endsWith_eq_endsWith_toSlice, Slice.toString_toSlice]

/-! ### Corollaries: trimAscii's toString has no surrounding whitespace -/
