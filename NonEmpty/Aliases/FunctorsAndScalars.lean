module

public import NonEmpty.Aliases.Scalars
public import NonEmpty.Aliases.Functors

public section
namespace NonEmpty.Aliases

abbrev «L/S» := L S
abbrev «NEL/S» := NEL S
abbrev «L/NES» := L NES
abbrev «NEL/NES» := NEL NES
abbrev «L/NESS» := L NESS
abbrev «NEL/NESS» := NEL NESS
abbrev «L/NEST» := L NEST
abbrev «NEL/NEST» := NEL NEST

abbrev «A/S» := A S
abbrev «NEA/S» := NEA S
abbrev «A/NES» := A NES
abbrev «NEA/NES» := NEA NES
abbrev «A/NESS» := A NESS
abbrev «NEA/NESS» := NEA NESS
abbrev «A/NEST» := A NEST
abbrev «NEA/NEST» := NEA NEST

abbrev «NEACBC/S» := NEACBC S
abbrev «NEACBC/NES» := NEACBC NES
abbrev «NEACBC/NESS» := NEACBC NESS
abbrev «NEACBC/NEST» := NEACBC NEST

end NonEmpty.Aliases
